...
import { userOne, userTwo, admin, insertUsers } from '../fixtures/userFixture';
import { adminAccessToken, userOneAccessToken } from '../fixtures/tokenFixture';
...
describe('User routes', () => {
  ...
  describe('GET /v1/users', () => {
    ...
    test('should return 401 if access token is missing', async () => {
      ...
      await request(app).get('/v1/users').send().expect(HTTP_CODE.UNAUTHORIZED);
    });
  ...
  describe('PATCH /v1/users/:userId', () => {
    test('should return 200 and successfully update user if data is ok', async () => {
      ...
      const res = await request(app)
        ...
        .set('Authorization', `Bearer ${userOneAccessToken}`)
        .send({ ...updateBody, password: hashedPassword })
        .expect(HTTP_CODE.OK);
    ...
    test('should return 400 if password length is less than 8 characters', async () => {
      await insertUsers([userOne]);
      const updateBody = { password: 'pass1' };
